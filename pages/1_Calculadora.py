...
with col_boleta:
    ...
    with aba_compra:
        with st.container(border=True):
            ...
            # --- 2. CARD VISUAL ESTILIZADO (Sem bugs de botão) ---
            st.markdown(f"""
                <div style="
                    display: flex; 
                    align-items: center; 
...
# ==========================================
# PAINEL INFERIOR (ESTRUTURA ORIGINAL RESTAURADA + ÍCONES)
# ==========================================
...
with col_abertos:
    ...
    if st.session_state['ordens_abertas']:
        for t in reversed(st.session_state['ordens_abertas']):
            ...
            # Estrutura HTML original detalhada + Injeção da imagem
            st.markdown(f"""
