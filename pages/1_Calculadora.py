...
# --- CONFIGURAÇÃO DE ATIVOS (LINKS CDN OFICIAIS) ---
ASSETS_CONFIG = {
...
with col_boleta:
    ...
    with aba_compra:
        with st.container(border=True):
            ...
            # --- CARD PREMIUM VISUAL (HTML Puro Corrigido) ---
            # Aqui estava o erro. As aspas foram ajustadas para garantir renderização.
            st.markdown(f"""
<div style="
    display: flex; 
    align-items: center; 
    ...
    with aba_venda:
        with st.container(border=True):
            if not st.session_state['ordens_abertas']:
                ...
                # CARD VISUAL NA VENDA (Corrigido)
                st.markdown(f"""
